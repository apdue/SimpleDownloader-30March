...
import com.apdue.simplenewdownloader_29march.dummy.VideoPlayerActivity;
...
public class VideosAdapterOnline extends RecyclerView.Adapter<VideosAdapterOnline.ViewHolder> {
    ...
    public int getItemCount() {
        if (new PrefManagerVideo(activity).getInt(SplashActivity.number_of_videos)>videos.size()){
            return videos.size();
        } else {
            return new PrefManagerVideo(activity).getInt(SplashActivity.number_of_videos);
        }
    }
